...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    percentage_generated_power = (generated_power/theoretical_max_power) * 100

    colour = 'black'
    if percentage_generated_power >= 80:
        colour = 'green'
    elif 60 <= percentage_generated_power < 80:
        colour = 'orange'
    elif 30 <= percentage_generated_power < 60:
        colour = 'red'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    percent_thresh_reached = (rate/threshold) * 100

    status = 'DANGER'
    if percent_thresh_reached < 90:
        status = 'LOW'
    elif 90 <= percent_thresh_reached <= 110:
        status = 'NORMAL'
